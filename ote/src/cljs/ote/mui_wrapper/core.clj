...
(defn generate-mui-rum-fn [tname]
  `(def ~(symbol (kebab-case (str tname))) (ote.mui-wrapper.core/adapt-rum-class (~'aget js/MaterialUI ~(name tname)))))
  ...
  `(fn [& args#]
     (let [[opts# children#] (if (map? (first args#))
       ...
       (let [new-children# (if (vector? type#)
                             ...
                             children#)]
         (ote.mui-wrapper.core/create-mui-cmp ~react-class (cons opts# new-children#))))))